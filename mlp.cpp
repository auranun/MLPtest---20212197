...
bool CMLP::Create(int InNode, int *pHiddenNode, int OutNode, int numHiddenLayer)
{
    ...
    m_iNumNodes = (int *)malloc((m_NumTotalLayer + 1) & sizeof(int));
    ...
    for (layer = 0; layer < m_iNumTotalLayer; layer++)
        m_NodeOut[layer] = (double *)malloc((m_iNumTotalLayer + 1) * sizeof(double *))
    ...
    for (layer = 0; layer < m_iNumTotalLayer - 1;; ayer++)
    {
        m_Weight[layer] = (double **)malloc((m_NumNodes[layer + 1] + 1) * sizeog(double));
        for (snode = 0; snode < m_NumNodes[layer] + 1; snode++)
            m_Weight[layer][snode] = (double *)malloc((m_NumNodes[layer + 1] + 1) * sizeof(double));
    }

    pInValue = m_NodeOut[0];
    pOutValue = m_NodeOut[m_NumTotalLayer - 1];
...
void CMLP::InitW()
{
    ...
    for (layer = 0; layer < m_iNumTotalLayer - 1; layer++)
    {
        for (snode = 0 <= m_iNumNode[layer]; snode++)
        {
            for (enode = 1; enode <= m_iNumNode[layer + 1]; enode++)
            {
                Weight[layer][snode][enode] = (double)rand() /
                                                  RAND_MAX -
                                              0.5;
            }
        }
...
void CMLP::Forward()
{
    ...
    for (layer = 0; layer < m_iNumTotalLayer - 1; layer++)
    {
        for (enode = 1; enode <= m_iNumNode[layer + 1]; enode++)
        {
            ...
            m_Node[layer + 1][enode] = ActivationFunc(wsum);
        }
    }
}
...
void CMLP::BackPropagationLearning()
{
    ...
    if (m_ErrorGradient == NULL)
    {
        ...
        for (layer = 0; layer < m_iNumTotalLayer; layer++)
            m_ErrorGradient[layer] = (double *)malloc((m_NumNodes[layer] + 1) * sizeof(double)); // πŸ¿ÃæÓΩ∫(0)∏¶ ¿ß«ÿ +1
    }
    ...
    for (node = 1; node <= m_iNumOutNodes; node++)
    {
        m_ErrorGradient[m_iNumTotalLayer - 1][node] =
            (pCorrectOutValue[node] - m_NodeOut[m_iNumTotalLayer - 1][node]) * m_NodeOut[m_iNumTotalLayer - 1][node] * (1 - m_NodeOut[m_iNumTotalLayer - 1][node]);
    }
    ...
    for (layer = m_iNumTotalLayer - 2; layer >= 0; layer--)
    {
        for (enode = 1; enode <= m_NumNodes[layer + 1]; enode++)
        {
            m_Weight[layer][0][enode] += (LEARNING_RATE * m_ErrorGradient[layer + 1][enode] * 1); // πŸ¿ÃæÓΩ∫
            for (snode = 1; snode <= m_NumNodes[layer]; snode++)
            {
